income=read.csv("C:/Users/Avinash Vallur/Desktop/Arjhun/Data Mining/Datasets/income_tr.csv")
...
for (i in 1:nrow(income))
{
  ...
  #caclualte distance for nominal data
      ...
      rdist = 
        (income$age[i]-income$age[j])^2 + 
        (income$capital_gain[i]-income$capital_gain[j])^2 +
        (income$capital_loss[i]-income$capital_loss[j])^2 +
        (income$hour_per_week[i]-income$hour_per_week[j])^2
      rdist=sqrt(rdist)
